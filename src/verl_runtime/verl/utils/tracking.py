...
class _TensorboardAdapter:
    ...
    def log(self, data, step):
        for key, value in data.items():
            # Tensorboard's add_scalar only accepts numeric values. A single
            # string metric (e.g. an accidentally-logged config key) would
            # otherwise crash the entire training run at metric time.
            if isinstance(value, (bool, int, float)):
                self.writer.add_scalar(key, float(value), step)
            elif hasattr(value, "item"):
                try:
                    self.writer.add_scalar(key, float(value.item()), step)
                except (TypeError, ValueError):
                    continue
        # Force-flush per-step so external monitors (modal volume get) see
        # progress in real time. SummaryWriter's default flush_secs (120s)
        # is unreliable for 4B (~64s/step) — writes can buffer for an entire
        # epoch on the worker before becoming visible on the Modal volume.
        self.writer.flush()
