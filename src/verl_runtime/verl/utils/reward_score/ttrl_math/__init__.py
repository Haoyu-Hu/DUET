...
This vendored copy keeps `latex2sympy2_extended` optional so the training
launcher can still run in environments that only have the core SymPy stack.
"""
...
def grade(model_answer: str, gt_answer: str, fast: bool = True):
    ...
    if not fast:
        # This mode uses the optional high-recall LaTeX verifier when available.
