...
except Exception:  # pragma: no cover - optional dependency on cluster
    latex2sympy = None
    ...
    try:
        with timeout(1):
            try:
                ...
                if parse is None or verify is None or ExprExtractionConfig is None or LatexExtractionConfig is None:
                    # Optional verifier packages are unavailable; fall back to the
                    # built-in symbolic path instead of crashing at import time.
                    return symbolic_equal(ground_truth, given_answer)
                return verify(
                    parse(
                        ground_truth,
                        extraction_config=(
