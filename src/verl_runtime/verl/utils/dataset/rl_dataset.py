...
class RLHFDataset(Dataset):
    ...
    def _add_suffix_to_entry(self, entry):
        # Append suffix_prompt to the last message's content.
        # Pandas + pyarrow may serialize the prompt column as a JSON string
        # (extension<arrow.json>) when the column contains Python objects.
        import json as _json
        prompt = entry[self.prompt_key]
        if isinstance(prompt, str):
            prompt = _json.loads(prompt)
        if isinstance(prompt, dict):
            # struct format: {"role": [...], "content": [...]}
            prompt["content"][-1] = prompt["content"][-1] + self.suffix_prompt
        else:
            # list-of-dicts format: [{"role": ..., "content": ...}, ...]
            prompt[-1]["content"] = prompt[-1]["content"] + self.suffix_prompt
        entry[self.prompt_key] = prompt
        return entry
    
    @staticmethod
    def _parse_json_strings(row_dict: dict) -> dict:
        """Deserialize columns that pyarrow serialized as JSON strings."""
        import json as _json
        for key in ("extra_info", "reward_model"):
            val = row_dict.get(key)
            if isinstance(val, str):
                try:
                    row_dict[key] = _json.loads(val)
    ...
    def __getitem__(self, item):
        ...
        row_dict = self._parse_json_strings(row_dict)
