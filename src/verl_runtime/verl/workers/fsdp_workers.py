...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # BSV actor_logits self-judge pass: bypass the rollout-temperature
        # override so the caller-supplied judge_temperature (default 1.0) is
        # honored. This is set ONLY by the BSV-GRPO overlay, never by the
        # main training path. See doc/bsv_methodology.md §5b + the 2026-04-21
        # review synthesis N3 (temperature contamination).
        if not data.meta_info.get("bsv_judge_pass", False):
