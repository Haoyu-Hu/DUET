...
class HFRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        # Accept **kwargs for compatibility with vLLM-style callers (e.g. TTRL
        # passes n=vote_size).  The 'n' kwarg overrides num_return_sequences
        # for the duration of this call.
        if "n" in kwargs:
            prompts.meta_info.setdefault("n_override", kwargs["n"])
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        if not do_sample:
            ...
            n_return = prompts.meta_info.get("n_override", self.config.n)
            kwargs = {
                "do_sample": True,
                "num_beams": 1,
                "top_p": top_p,
